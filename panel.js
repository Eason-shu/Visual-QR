(() => {
  ...
  let currentTemplateId = (templates[0] ? templates[0].id : null) || createTemplate('默认模板').id;
  let scanTimer = null;
  let scanBuffer = '';
  let scanLastAt = 0;
    ...
    delays.forEach((delay) => {
      setTimeout(() => {
        try {
          ...
          if (window.__qrfillHiddenScanner.select) window.__qrfillHiddenScanner.select();
  ...
  function uid() { return 'tpl_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8); }
  function loadTemplates() {
    try { return JSON.parse(localStorage.getItem(STATE_KEY) || '[]'); } catch { return []; }
  }
  function saveTemplates() { localStorage.setItem(STATE_KEY, JSON.stringify(templates)); }
  function createTemplate(name) {
    const tpl = { id: uid(), name: name || '未命名模板', pageKey: pageKey(), fields: [] };
    templates.push(tpl); saveTemplates(); return tpl;
  }
  ...
  function currentTemplate() { return templates.find(t => t.id === currentTemplateId) || templates[0]; }
  ...
  function cssPath(el) {
    ...
    while (el && el.nodeType === 1 && el !== document.body) {
      ...
      if (el.name) { part += `[name="${CSS.escape(el.name)}"]`; parts.unshift(part); break; }
  ...
  function byXPath(xpath) {
    try { return document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; } catch { return null; }
  }
  ...
  function isActuallyVisible(el) {
    ...
    while (node && node.nodeType === 1 && node !== document.body) {
      ...
      if (/\b(is-hidden|hidden|el-tab-pane--hidden|ant-tabs-tabpane-hidden)\b/.test(cls)) return false;
    ...
    if (rect.width === 0 && rect.height === 0 && !el.isContentEditable) return false;
  ...
  function findFieldScope(el) {
    ...
    if (scope && scope !== document.body && scope !== document.documentElement) return scope;
    return null;
  }
  ...
  function scopeRoots(field) {
    ...
    if (field.scopeSelector) {
      try {
        roots.push(...Array.from(document.querySelectorAll(field.scopeSelector)).filter(isActuallyVisible));
      } catch {}
    }
  ...
  function bestVisible(candidates) {
    ...
    return visible
      .map(el => ({ el, score: (isInViewport(el) ? 100 : 0) + (el === document.activeElement ? 10 : 0) }))
  ...
  function getLabelText(el) {
    ...
    const item = el.closest('.el-form-item,.ant-form-item,.n-form-item,.form-item,.form-group,label');
    if (item) {
      const label = item.querySelector('label,.el-form-item__label,.ant-form-item-label,.n-form-item-label');
  ...
  function realFillable(target) {
    ...
    return target.querySelector ? target.querySelector('input,textarea,select,[contenteditable="true"]') : null;
  }
  ...
  function locate(field) {
    ...
    if (field.placeholder) {
      selectors.push(`input[placeholder*="${CSS.escape(field.placeholder)}"],textarea[placeholder*="${CSS.escape(field.placeholder)}"]`);
    }
    ...
    for (const root of scopeRoots(field)) {
      ...
      if (field.xpath) {
        ...
        if (fillable && isActuallyVisible(fillable) && (root === document || root.contains(fillable))) {
          return fillable;
        }
      }
    }
  ...
  function nativeSet(el, value) {
    ...
    ['input','change','blur','keyup'].forEach(n => el.dispatchEvent(new Event(n, { bubbles: true })));
  }

  async function fillSelectComponent(el, value) {
    const text = String(value);
    const root = el.closest('.el-select,.ant-select,.n-select');
    if (!root) return false;
    const clickable = root.querySelector('.el-input,.el-select__wrapper,.ant-select-selector,.n-base-selection') || root;
    clickable.click(); if (el.focus) el.focus();
    await sleep(250);
    ...
    const matched = options.find(o => o.textContent.trim() === text) || options.find(o => o.textContent.trim().includes(text));
  ...
  async function smartFill(el, value) {
    if (!el) return false;
    if (el.closest('.el-select,.ant-select,.n-select')) {
      const ok = await fillSelectComponent(el, value);
      if (ok) return true;
    }
    const tag = el.tagName.toLowerCase();
    if (tag === 'select') { el.value = String(value); el.dispatchEvent(new Event('change', { bubbles: true })); return true; }
    if (el.type === 'checkbox') { el.checked = Boolean(value); el.dispatchEvent(new Event('change', { bubbles: true })); return true; }
    if (el.type === 'radio') {
      const radio = document.querySelector(`input[type="radio"][name="${CSS.escape(el.name)}"][value="${CSS.escape(String(value))}"]`);
      if (radio) { radio.checked = true; radio.dispatchEvent(new Event('change', { bubbles: true })); return true; }
    }
    if (el.isContentEditable) { el.textContent = String(value); el.dispatchEvent(new Event('input', { bubbles: true })); return true; }
    nativeSet(el, value);
    return true;
  }
  ...
  function parsePayload(raw) {
    ...
    if (cleaned.startsWith('FORMQR64:')) {
      const encoded = cleaned.slice('FORMQR64:'.length).trim().replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, '');
      const json = base64UrlToUtf8(encoded);
  ...
  async function fillByPayload(payload) {
    ...
    if (payload && typeof payload === 'object' && payload.data) {
      data = payload.data;
      if (payload.templateId) tpl = templates.find(t => t.id === payload.templateId) || tpl;
      if (payload.template) tpl = templates.find(t => t.name === payload.template) || tpl;
    }
    if (!tpl) { toast('未找到模板', '请先创建并保存模板'); return; }
    const results = [];
    let successCount = 0;
    let failCount = 0;
    let skipCount = 0;
    ...
    for (const field of tpl.fields) {
      if (!Object.prototype.hasOwnProperty.call(data, field.key)) {
        results.push({ key: field.key, status: '跳过', msg: '二维码无此字段' });
        skipCount++;
        continue;
      }
      ...
      if (!el) {
        results.push({ key: field.key, status: '失败', msg: '未找到元素' });
        failCount++;
        continue;
      }
      ...
      results.push({ key: field.key, status: ok ? '成功' : '失败', msg: String(data[field.key]) });
    }
    
    renderFields();
    const summary = `成功 ${successCount} 个，失败 ${failCount} 个${skipCount > 0 ? `，跳过 ${skipCount} 个` : ''}`;
    toast('填充完成', summary);
  }
  ...
  async function fetchFromService() {
    ...
    try {
      ...
      // 解析数据并填充
      await fillByPayload(data.data || data);
    } catch (e) {
  ...
  function switchMode(mode) {
    ...
    updateModeUI();
    toast('模式切换', mode === 'scanner' ? '已切换到扫码枪模式' : '已切换到后台服务模式');
  }
  ...
  function updateModeUI() {
    ...
    if (serviceSection) {
      serviceSection.style.display = currentMode === 'service' ? 'grid' : 'none';
    }
    
    const scannerSection = document.getElementById('scannerSection');
    if (scannerSection) {
      scannerSection.style.display = currentMode === 'scanner' ? 'grid' : 'none';
    }
  ...
  function charFromKeyboardEvent(e) {
    ...
    const map = {
      Space: ' ', Minus: e.shiftKey ? '_' : '-', Equal: e.shiftKey ? '+' : '=',
      BracketLeft: e.shiftKey ? '{' : '[', BracketRight: e.shiftKey ? '}' : ']',
      Backslash: e.shiftKey ? '|' : '\\', Semicolon: e.shiftKey ? ':' : ';',
      Quote: e.shiftKey ? '"' : "'", Comma: e.shiftKey ? '<' : ',',
      Period: e.shiftKey ? '>' : '.', Slash: e.shiftKey ? '?' : '/',
  ...
  function commitScanBuffer(reason) {
    ...
    if (raw.includes('QRFILL1:') || raw.includes('FORMQR') || raw.includes('FORMQR64:')) handleScan(raw);
  }
  ...
  function pushScanChar(ch) {
    ...
    scanBuffer += ch;
    clearTimeout(scanTimer);
    scanTimer = setTimeout(() => commitScanBuffer('延迟提交'), SCAN_AUTO_COMMIT_DELAY);
  }
  ...
  function onKeydown(e) {
    ...
    if (e.key === 'Enter' || e.key === 'Tab') {
      if (scanBuffer.trim()) { e.preventDefault(); commitScanBuffer('结束符 ' + e.key); }
      return;
    }
    const ch = charFromKeyboardEvent(e) || (e.key && e.key.length === 1 ? e.key : '');
    if (ch) pushScanChar(ch);
  }
  function isPanelTarget(el) { return !!(el && el.closest && el.closest('#' + panelId)); }
  // 只监听扫描枪键盘事件；不监听剪贴板/复制粘贴事件，避免 QRFILL1 内容区复制文本时触发填充。
  document.addEventListener('keydown', onKeydown, true);

  function selectElement(e) {
    if (isPanelTarget(e.target)) return;
    const fillable = realFillable(e.target) || realFillable(e.target.closest ? e.target.closest('.el-select,.ant-select,.n-select,.el-input,.ant-input-affix-wrapper,.n-input') : null);
    if (!fillable) return;
    if (selectedElement) selectedElement.classList.remove('__qrfill_selected__');
  ...
  style.textContent = `
    ...
    #${panelId} .titleBox{display:grid;gap:2px;min-width:0}
    #${panelId} .title{font-weight:850;font-size:22px;letter-spacing:.01em;color:#101828;line-height:1.1}
  ...
  function enablePanelDrag() {
    ...
    handle.addEventListener('pointermove', (event) => {
      if (!dragging) return;
      const next = clampPanelPosition(startLeft + event.clientX - startX, startTop + event.clientY - startY);
      panel.style.left = next.left + 'px';
      panel.style.top = next.top + 'px';
      event.preventDefault();
    });
    ...
    function stopDrag(event) {
      ...
      try { if (handle.releasePointerCapture) handle.releasePointerCapture(event.pointerId); } catch {}
    }
  ...
  function commitInputScanner(input, name, reason) {
    ...
    const raw = codeRaw.includes('QRFILL1:') || codeRaw.includes('FORMQR') ? codeRaw : valueRaw;
  ...
  function bindScannerInput(input, name) {
    ...
    input.addEventListener('compositionstart', e => {
      ...
      input.value = '';
      log('输入法防护', name + ' 检测到 compositionstart，忽略输入法组合内容');
    }, true);
    ...
    input.addEventListener('keydown', e => {
      ...
      if (ch) {
        ...
        input.__qrfillTimer = setTimeout(() => {
          ...
          if (raw.includes('QRFILL1:') || raw.includes('FORMQR')) commitInputScanner(input, name, 'ASCII code 延迟收包');
  ...
  document.addEventListener('focusin', (e) => {
    ...
    if (!lastScanCompletedAt || Date.now() - lastScanCompletedAt > 5000) return;
  ...
  function toast(title, msg) {
    const old = document.querySelector('.qrfill-toast'); if (old) old.remove();
    const div = document.createElement('div'); div.className = 'qrfill-toast'; div.textContent = `${title}\n${msg || ''}`;
    document.body.appendChild(div); setTimeout(() => div.remove(), 7000);
  }
  function renderTemplates() {
    const sel = document.getElementById('tplSelect');
    sel.innerHTML = templates.map(t => `<option value="${t.id}">${escapeHtml(t.name)}｜${escapeHtml(t.pageKey)}</option>`).join('');
    sel.value = currentTemplateId;
    var tplObj = currentTemplate();
    document.getElementById('tplName').value = (tplObj ? tplObj.name : '') || '';
    renderFields();
  }
  ...
  function renderFields() {
    ...
    if (!tpl || !tpl.fields.length) { box.innerHTML = '<div class="small">暂无字段绑定，先点击页面上的表单框，再输入二维码字段 key 进行绑定。</div>'; return; }
    box.innerHTML = tpl.fields.map((f, i) => `
      <div class="item">
  ...
  function updateSelectedInfo() {
    ...
    const info = el ? `已选中表单框：${el.tagName.toLowerCase()} ｜ name=${el.name||'-'} ｜ id=${el.id||'-'} ｜ placeholder=${el.placeholder||'-'} ｜ label=${getLabelText(el)||'-'}` : '未选中字段，请直接点击页面上的输入框、下拉框或文本域。';
  ...
  function extractSelectOptions(el) {
    ...
    if (tag === 'select') {
      return Array.from(el.options || []).map(o => ({ value: o.value, text: o.textContent.trim() })).filter(o => o.value);
    }
    const wrapper = el.closest('.el-select,.ant-select,.n-select');
    if (!wrapper) return [];
    const selectEl = wrapper.querySelector('select');
    if (selectEl) {
      return Array.from(selectEl.options || []).map(o => ({ value: o.value, text: o.textContent.trim() })).filter(o => o.value);
    }
    return [];
  }
  ...
  function suggestKeys(el) {
    ...
    const autoMap = {
      'name': 'xm', 'given-name': 'x', 'family-name': 'm',
      'email': 'yx', 'tel': 'dh', 'phone': 'sj',
      'street-address': 'dz', 'postal-code': 'yb',
      'organization': 'gs', 'country': 'gj',
      'username': 'yhm', 'password': 'mm', 'new-password': 'xmm',
      'bday': 'sr', 'sex': 'xb', 'gender': 'xb',
      'cc-name': 'ckr', 'cc-number': 'kh', 'cc-exp': 'yxq',
    };
    const autoVal = autoMap[autocomplete.toLowerCase()];
    if (autoVal) add(autoVal, 'autocomplete');

    const typeMap = {
      'email': 'yx', 'tel': 'dh', 'url': 'wz',
      'date': 'rq', 'time': 'sj', 'number': 'sl',
      'color': 'ys', 'password': 'mm',
    };
    const typeVal = typeMap[type];
    if (typeVal) add(typeVal, 'type');

    if (placeholder) {
      const phCleaned = placeholder.replace(/请输入|请填写|选择|填写|：|:/g, '').trim();
      if (phCleaned && phCleaned !== label) {
        add(phCleaned, 'placeholder');
        const phPinyinMap = {
          '姓名': 'xm', '手机号': 'sjh', '邮箱': 'yx', '地址': 'dz',
          '身份证号': 'sfzh', '编号': 'bh', '备注': 'bz', '描述': 'ms',
          '密码': 'mm', '用户名': 'yhm', '验证码': 'yzm',
          '公司名称': 'gsmc', '部门': 'bm',
        };
  ...
  function showSuggestBox(el) {
    ...
    for (const s of suggestions) {
      ...
      btn.onclick = () => {
        keys.querySelectorAll('.suggest-key').forEach(b => b.classList.remove('active'));
  ...
  function findRecordableTarget(target) {
    ...
    const fillable = realFillable(target)
      || realFillable(target.closest ? target.closest('.el-select,.ant-select,.n-select,.el-input,.ant-input-affix-wrapper,.n-input') : null);
    return fillable;
  }
  ...
  function onRecordClick(e) {
    ...
    log('录制抓取', `元素=${target.tagName} label="${label}" name="${name}" id="${target.id}"`);
  ...
  function startRecording() {
    ...
    if (text) text.innerHTML = '<b>录制中</b> — 点击页面上的表单框自动抓取并推荐 key';
    if (btn) { btn.innerHTML = '<svg viewBox="0 0 24 24" fill="currentColor" style="width:14px;height:14px"><rect x="7" y="7" width="10" height="10" rx="1"/></svg> 停止录制'; btn.className = 'record-off'; }
  ...
  function stopRecording() {
    ...
    if (recordHoverEl) { recordHoverEl.classList.remove('__qrfill_hover__'); recordHoverEl = null; }
    hideRecordTooltip();
    hideSuggestBox();
    const dot = document.getElementById('recordDot');
    const text = document.getElementById('recordText');
    ...
    if (text) text.innerHTML = '点击右侧按钮开启<b>录制模式</b>，自动抓取页面元素并推荐 key';
    if (btn) { btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px"><circle cx="12" cy="12" r="7" fill="#ef4444"/><circle cx="12" cy="12" r="4"/></svg> 开启录制'; btn.className = 'record-on'; }
    log('录制模式', '已停止');
    toast('录制模式已停止', '');
  }
  ...
  function generateValueByPattern(pattern, maxLength) {
    ...
    const charSets = {
      ...
      '\\w': () => 'abcdefghijklmnopqrstuvwxyz0123456789'[Math.floor(Math.random() * 36)],
      '\\s': () => ' ',
      '[0-9]': () => '0123456789'[Math.floor(Math.random() * 10)],
      '[a-zA-Z]': () => 'abcdefghijklmnopqrstuvwxyz'[Math.floor(Math.random() * 26)],
      '[a-z]': () => 'abcdefghijklmnopqrstuvwxyz'[Math.floor(Math.random() * 26)],
      '[A-Z]': () => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[Math.floor(Math.random() * 26)],
      '[a-zA-Z0-9]': () => 'abcdefghijklmnopqrstuvwxyz0123456789'[Math.floor(Math.random() * 36)],
    };
        ...
        if (ch === '^' || ch === '$' || ch === '(' || ch === ')' || ch === '|' || ch === '?' || ch === '*' || ch === '+' || ch === '{') {
          ...
          if (end > i) {
            ...
            if (range.includes('-')) {
              ...
              result += String.fromCharCode(Math.floor(Math.random() * (endChar.charCodeAt(0) - start.charCodeAt(0) + 1)) + start.charCodeAt(0));
            } else {
  ...
  function generateMockData() {
    const tpl = currentTemplate();
    if (!tpl || !tpl.fields.length) { toast('自动生成', '当前模板无字段，请先绑定字段'); return; }
    ...
    for (const f of tpl.fields) {
      ...
      if (val !== null) { stats.byType++; obj[f.key] = val; continue; }

      if (f.pattern) {
        val = generateValueByPattern(f.pattern, f.maxLength);
        if (val !== null) { stats.byPattern++; obj[f.key] = val; continue; }
      }

      val = generateValueByKeyword(f);
      if (val !== null) { stats.byKeyword++; obj[f.key] = val; continue; }
  ...
  panel.querySelector('#qrClose').onclick = () => { if (recordingMode) stopRecording(); panel.style.display = 'none'; focusHiddenScanner(); toast('面板已隐藏', '扫码监听仍在当前页面生效；刷新页面后也会自动恢复监听'); };
  panel.querySelector('#tplSelect').onchange = e => { currentTemplateId = e.target.value; renderTemplates(); };
  panel.querySelector('#newTpl').onclick = () => { const name = prompt('模板名称', '新模板'); if (!name) return; currentTemplateId = createTemplate(name).id; renderTemplates(); };
  panel.querySelector('#saveTpl').onclick = () => { const tpl = currentTemplate(); tpl.name = document.getElementById('tplName').value || tpl.name; tpl.pageKey = pageKey(); saveTemplates(); renderTemplates(); toast('已保存模板', tpl.name); };
  panel.querySelector('#delTpl').onclick = () => { if (!confirm('确定删除当前模板？')) return; templates = templates.filter(t => t.id !== currentTemplateId); if (!templates.length) createTemplate('默认模板'); currentTemplateId = templates[0].id; saveTemplates(); renderTemplates(); };
  panel.querySelector('#exportTpl').onclick = () => { const tpl = currentTemplate(); if (!tpl) return; const blob = new Blob([JSON.stringify(tpl, null, 2)], {type: 'application/json'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = (tpl.name || 'template') + '.json'; a.click(); URL.revokeObjectURL(url); toast('已导出模板', tpl.name); };
  panel.querySelector('#importTpl').onclick = () => { const input = document.createElement('input'); input.type = 'file'; input.accept = '.json'; input.onchange = e => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = ev => { try { const tpl = JSON.parse(ev.target.result); if (!tpl.name || !Array.isArray(tpl.fields)) throw new Error('无效模板格式'); tpl.id = Date.now().toString(); templates.push(tpl); currentTemplateId = tpl.id; saveTemplates(); renderTemplates(); toast('已导入模板', tpl.name); } catch(err) { toast('导入失败', err.message); } }; reader.readAsText(file); }; input.click(); };
  panel.querySelector('#bindField').onclick = () => {
    const el = selectedElement; const key = document.getElementById('fieldKey').value.trim(); const tpl = currentTemplate();
    if (!el || !key || !tpl) return toast('无法绑定', '请先点击页面字段，并输入 key');
    ...
    const field = {
      ...
      component: el.closest('.el-select') ? 'element-select' : el.closest('.ant-select') ? 'ant-select' : el.closest('.n-select') ? 'naive-select' : '',
      visibleOnly: true
    };
    const idx = tpl.fields.findIndex(f => f.key === key);
    if (idx >= 0) tpl.fields[idx] = field; else tpl.fields.push(field);
    saveTemplates(); renderFields(); toast('已绑定字段', key);
  };
  panel.querySelector('#testFill').onclick = () => { try { fillByPayload(JSON.parse(document.getElementById('testJson').value)); } catch(e) { toast('测试 JSON 错误', e.message); } };
  panel.querySelector('#genAscii').onclick = () => { try { const obj = JSON.parse(document.getElementById('testJson').value); const out = 'QRFILL1:' + utf8ToBase64Url(JSON.stringify(obj)); document.getElementById('asciiOut').value = out; log('生成 QRFILL1', out); } catch(e) { toast('生成失败', e.message); } };
  panel.querySelector('#autoGenMock').onclick = () => { generateMockData(); };
  const visibleScanner = panel.querySelector('#scannerInput');
  ['autocomplete','autocorrect','autocapitalize','spellcheck'].forEach(attr => visibleScanner.setAttribute(attr, attr === 'spellcheck' ? 'false' : 'off'));
  visibleScanner.setAttribute('inputmode', 'latin');
  visibleScanner.setAttribute('lang', 'en');
  bindScannerInput(visibleScanner, '面板捕获框');
  panel.querySelector('#focusScanner').onclick = () => { visibleScanner.focus(); visibleScanner.select(); toast('已聚焦扫码捕获框', '现在可以扫码'); };
  panel.querySelector('#recordToggle').onclick = () => { if (recordingMode) stopRecording(); else startRecording(); };
  ...
  fab.id = 'qrfill-fab';
  fab.innerHTML = '<svg viewBox="0 0 24 24"><path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4z"/><path d="M15 15h2v2h-2zM19 15h1v5h-5v-1M14 19v1"/></svg>';
  fab.title = '展开/收起面板';
  let fabDragging = false, fabDragOffsetX = 0, fabDragOffsetY = 0;
  fab.addEventListener('mousedown', e => { fabDragging = true; fabDragOffsetX = e.clientX - fab.getBoundingClientRect().left; fabDragOffsetY = e.clientY - fab.getBoundingClientRect().top; e.preventDefault(); });
  document.addEventListener('mousemove', e => { if (!fabDragging) return; fab.style.right = 'auto'; fab.style.bottom = 'auto'; fab.style.left = (e.clientX - fabDragOffsetX) + 'px'; fab.style.top = (e.clientY - fabDragOffsetY) + 'px'; });
  document.addEventListener('mouseup', () => { fabDragging = false; });
  fab.addEventListener('click', () => { window.__QRFILL_ASCII_PANEL__.toggle(); });
  document.body.appendChild(fab);
  // 初始化
  loadSettings();
  renderTemplates();
  updateModeUI();
  log('启动', `v${APP_VERSION} 页面加载后已自动恢复扫码枪键盘监听；支持后台服务模式`);
  setTimeout(focusHiddenScanner, 300);
})();
